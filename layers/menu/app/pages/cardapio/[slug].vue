...
import { ArrowLeft, ChevronDown, ImageOff, Search, SearchX, Star, X } from '@lucide/vue'
...
const promoProducts = computed(() => filteredProducts.value.filter((product) => product.promoPrice != null))
...
<template>
  <main v-if="menu" class="mx-auto min-h-svh max-w-lg bg-background pb-20 lg:max-w-5xl">
    <header
    ...
    >
      ...
      <div class="relative flex flex-col items-center gap-1.5">
        ...
        <div class="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-sm text-white/85">
          <span v-if="menu.establishment.segment">{{ menu.establishment.segment }}</span>
    ...
    <div v-if="menu.products.length === 0" class="flex flex-col items-center gap-3 px-5 py-20 text-center">
      <ImageOff class="size-8 text-muted-foreground/50" />
      <p class="text-sm text-muted-foreground">Nenhum item disponível no momento.</p>
    </div>

    <div v-else-if="filteredProducts.length === 0" class="flex flex-col items-center gap-3 px-5 py-20 text-center">
      <SearchX class="size-8 text-muted-foreground/50" />
      <p class="text-sm text-muted-foreground">Nenhum item encontrado para "{{ searchQuery }}".</p>
    </div>

    <template v-else>
      <section v-if="promoProducts.length > 0" class="pt-6">
        <h2 class="mb-3 px-5 text-sm font-semibold uppercase tracking-wide text-muted-foreground lg:px-8">
          Promoções
        </h2>
        <ul class="-mx-5 flex gap-3 overflow-x-auto px-5 pb-1 [scrollbar-width:none] lg:mx-0 lg:px-8 [&::-webkit-scrollbar]:hidden">
          <li
          ...
          >
            <div class="relative flex aspect-[16/10] items-center justify-center bg-muted">
              <img
                ...
                loading="lazy"
              >
              <span v-else class="flex size-full items-center justify-center bg-gradient-to-br from-accent to-secondary text-2xl font-semibold text-accent-foreground">
              ...
              <div class="absolute inset-x-0 bottom-0 p-3">
                ...
                <div class="mt-0.5 flex items-baseline gap-1.5">
                  <span class="text-xs text-white/60 line-through">{{ formatCurrency(product.price) }}</span>
                  <span class="text-sm font-bold text-white">{{ formatCurrency(product.promoPrice!) }}</span>
                </div>
              </div>
            </div>
          </li>
      ...
      >
        ...
        >
          <h2 class="text-sm font-semibold uppercase tracking-wide text-muted-foreground">
            {{ category }}
            <span class="ml-1 font-normal text-muted-foreground/60">({{ products.length }})</span>
          </h2>
          <ChevronDown
        ...
        <ul v-show="!collapsedCategories[category]" class="grid grid-cols-1 gap-2.5 lg:grid-cols-2">
          <li
            ...
            @click="openDetail(product)"
          >
            <div class="relative size-24 shrink-0 overflow-hidden rounded-xl bg-muted">
              <img
                v-if="product.imageUrl"
                :src="product.imageUrl"
                :alt="product.name"
                ...
                loading="lazy"
              >
              <span v-else class="flex size-full items-center justify-center bg-gradient-to-br from-accent to-secondary text-xl font-semibold text-accent-foreground">
                {{ product.name.charAt(0) }}
              </span>
              <span
                v-if="discountPercent(product)"
            ...
            <div class="flex min-w-0 flex-1 flex-col justify-between py-0.5">
              <div class="min-w-0">
                <p class="font-semibold leading-snug text-foreground">{{ product.name }}</p>
                <p v-if="product.description" class="mt-0.5 line-clamp-2 text-sm text-muted-foreground">
                  {{ product.description }}
                </p>
              </div>
      ...
      >
        <div class="relative h-60 shrink-0 overflow-hidden bg-muted sm:h-56">
          <img
            ...
            class="size-full object-cover"
          >
          <div v-else class="flex size-full items-center justify-center bg-gradient-to-br from-accent to-secondary">
            <span class="text-4xl font-semibold text-accent-foreground">{{ selectedProduct.name.charAt(0) }}</span>
          </div>
          <DialogClose
        ...
        <div class="-mt-5 flex-1 space-y-4 overflow-y-auto rounded-t-3xl bg-background p-5 sm:mt-0 sm:rounded-none sm:p-6">
          <div class="mx-auto h-1.5 w-10 rounded-full bg-muted sm:hidden" />

          <DialogHeader class="space-y-1.5 text-left">
            <p v-if="selectedProduct.category" class="text-xs font-semibold uppercase tracking-wider text-primary">
              {{ selectedProduct.category }}
            </p>
            <DialogTitle class="text-xl font-bold leading-snug">{{ selectedProduct.name }}</DialogTitle>
          </DialogHeader>

          <p v-if="selectedProduct.description" class="text-sm leading-relaxed text-muted-foreground">
